...
parser.add_argument('--dataset_name', type=str, default='GlaS', help='Name of the dataset')
...
# 读取 JSON 文件
with open(dataset + '/data_split.json') as f:
    data = json.load(f)
...
plt.title('File Count in Train, Valid, and Test Datasets')
...
# 保存图像
plt.savefig(f'./data_analysis_result/{dataset}_data_split_histogram.png')
...
for split in SPLIT:
    ...
    axs[0, 0].set_title(f'Object Size Distribution in {split} set')
    # 在每个柱子上添加计数值
    ...
    axs[0, 1].set_title(f'Object Density Distribution in {split} set')
    # 在每个柱子上添加计数值
    for count, patch in zip(counts, patches):
        x = patch.get_x() + patch.get_width() / 2
        y = patch.get_height()
    ...
    axs[1, 0].set_title(f'Category Distribution in {split} set')
    ...
    if 1 < len(unique_sizes) < 20:
        ...
        axs[1, 1].set_title(f'Image Size Distribution in {split} set')
    else:
        # 图像尺寸分布
    ...
    plt.tight_layout()
    # 保存图像
    fig.savefig(f'./data_analysis_result/{dataset}_{split}_analysis.png', dpi=300)
...
axs[0, 0].set_xlabel('Object Size')
axs[0, 0].set_ylabel('Count')
axs[0, 0].set_title(f'Object Size Distribution')
# 在每个柱子上添加计数值
for count, patch in zip(counts, patches):
    x = patch.get_x() + patch.get_width() / 2
    y = patch.get_height()
...
axs[0, 1].set_xlabel('Object Density')
axs[0, 1].set_ylabel('Count')
axs[0, 1].set_title(f'Object Density Distribution')
# 在每个柱子上添加计数值
for count, patch in zip(counts, patches):
    x = patch.get_x() + patch.get_width() / 2
    y = patch.get_height()
...
axs[1, 0].set_xlabel('Category')
axs[1, 0].set_ylabel('Count')
axs[1, 0].set_title(f'Category Distribution')
axs[1, 0].set_xticks(range(len(category_counts)))
axs[1, 0].set_xticklabels(categories, rotation=90)
# 在每个柱子上添加计数值
for i, count in enumerate(class_counts):
...
if 1 < len(unique_sizes) < 20:
    ...
    axs[1, 1].set_xlabel('Image Size')
    axs[1, 1].set_ylabel('Frequency')
    axs[1, 1].set_title(f'Image Size Distribution')
else:
    # 图像尺寸分布
    axs[1, 1].scatter(image_sizes[:, 1], image_sizes[:, 0], marker='.')
    axs[1, 1].set_xlabel('Image Width')
...
plt.tight_layout()
# 保存图像
fig.savefig(f'./data_analysis_result/{dataset}_analysis.png', dpi=300)
